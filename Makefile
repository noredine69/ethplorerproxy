...
build: ## Build the docker image
	@docker image build \
			...
			--build-arg PORT_REST_API_ARG=$(PORT_REST_API) \
...
start:
	@docker run -p $(PORT_REST_API):$(PORT_REST_API) $(IMAGE_CURRENT)

start-latest:
	@docker run -p $(PORT_REST_API):$(PORT_REST_API) $(IMAGE_LATEST)
